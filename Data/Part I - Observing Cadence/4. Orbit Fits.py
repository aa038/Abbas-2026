...
def fit_all_planets_in_log(log_path: str | Path, cadence_name: str) -> pd.DataFrame:
    ...
    for planet_idx, planet_id in enumerate(obs_df["PlanetID"].unique()):
        ...
        # Fit progressively as each detected epoch is added
        fit_results = []
        diagnostics_by_detection = []
        previous_orbits = None

        for n_det in range(1, len(det_times) + 1):
            times_now = np.asarray(det_times[:n_det], dtype=float)
            seps_now  = np.asarray(det_seps[:n_det], dtype=float)
            pas_now   = np.asarray(det_pas[:n_det], dtype=float)

            sep_uncertainties = np.full(n_det, SEP_UNCERTAINTY, dtype=float)
            pa_uncertainties  = np.degrees(sep_uncertainties / seps_now)

            try:
                orbits, diagnostics = fit_orbit(
                    times=times_now,
                    separations=seps_now,
                    position_angles=pas_now,
                    stellar_mass=m_star,
                    distance=d_pc,
                ...
                print(f"[ERROR] Fit failed for {planet_id} at detection {n_det}: {e}")
                fit_results.append(None)
                diagnostics_by_detection.append(None)
                continue

            fit_results.append(orbits)
            diagnostics_by_detection.append(diagnostics)
