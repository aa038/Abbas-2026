...
Output:
    1a. Observing Log.csv                                 # The observing log for the chosen telescope configuration
"""
...
CONTRAST_FLOOR = 1.0e-9         # Contrast floor for this fiducial-case directory
...
def run_mission(save_csv=True, verbose=True):
    ...
    if save_csv:
        ...
        out.to_csv(curr_dir / "1a. Observing Log.csv", index=False)
        if verbose:
            print(f"\nSaved {len(out)} rows to 1a. Observing Log.csv")
