...
def IntegrationTime(L, d, A, throughput = 0.27, contrast = 1e-9, SNR = 10, wavelength = 0.6e-6):
...
def contrastCheck(DetParams, albedo, Rp, coronagraphContrast = 1e-9):
...
def SNRCheck(L, d, Rp, albedo, A, tExp, DetParams, wavelength = 0.6e-6, throughput = 0.27, contrast = 1e-9):
...
ObsLog.to_csv(curr_dir / "3a. Observing Log.csv", index=False)
