"""
Observe the mock planet catalog (Part 1 of 9)
-------------------------------------------
This script observes the full planet catalog under the adaptive cadence for demographics studies.
...
Output:
    4a. Observing Log.csv                                 # The observing log for the chosen telescope configuration
"""
...
IWA            = 0.04           # Inner working angle in arcsec
...
P_DET_MIN      = 0.50           # Minimum preferred detectability fraction for an epoch 
...
def run_mission(save_csv=True, verbose=True):
    ...
    if save_csv:
        ...
        out.to_csv(curr_dir / "4a. Observing Log.csv", index=False)
        if verbose:
            print(f"\nSaved {len(out)} rows to 4a. Observing Log.csv")
