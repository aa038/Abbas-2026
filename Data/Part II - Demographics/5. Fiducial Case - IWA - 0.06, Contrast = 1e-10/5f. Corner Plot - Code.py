...
df['freq_earth_radius'] = df['freq'] * (1.4 ** (df['alpha']+1) - 0.8 ** (df['alpha']+1)) / (3.4 ** (df['alpha']+1) - 0.5 ** (df['alpha']+1))

# Compute the exo-Earth OR for a 1 M_sol star
df['eta_Earth'] = [100.0 * row['freq_earth_radius'] * hz_fraction(row) for _, row in df.iterrows()]
# ------------------------------------------------------------------------------ #
...
eta_Earth = eta_Earth * (2.35 ** (beta+1) - 0.65 ** (beta+1)) / (10 ** (beta+1) - 0.03 ** (beta+1))
