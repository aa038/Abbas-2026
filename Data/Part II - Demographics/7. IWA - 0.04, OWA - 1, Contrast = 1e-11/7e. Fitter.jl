...
Input:
    7b. 4D Tongue Plot.npz              # The 4D tongue plot stored a 4D NumPy array (From Part b)
    7d. Planet Posterior Samples.csv    # Final Orbituary P/e posterior samples (From Part d)


Output:
    7e. Fit, N = 1e4.csv                # The full list of MCMC posteriors for each parameter in the OR model
"""
...
"""
    ...
    tplot_file   = joinpath(curr_dir, "7b. 4D Tongue Plot.npz")
...
posterior_samples_file = joinpath(curr_dir, "7d. Planet Posterior Samples.csv")
...
CSV.write(joinpath(curr_dir, "7e. Fit, N = 1e4.csv"), df)
