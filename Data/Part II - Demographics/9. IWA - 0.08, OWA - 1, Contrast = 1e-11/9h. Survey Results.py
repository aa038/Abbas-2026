...
planets_df = pd.read_csv(curr_dir / "9d. Detected Planets.csv")
...
mock_survey = pd.read_csv(os.path.join(curr_dir, '9g. Mock Survey.csv'))
...
bins = np.arange(0, max(num_planets) + 2, 10)
...
axs[4].set_xlim([160,380])

axs[4].xaxis.set_major_locator(ticker.MultipleLocator(40))
axs[4].xaxis.set_minor_locator(ticker.MultipleLocator(20))
...
bins = np.arange(0, max(num_HZ_planets) + 2, 2)
...
plt.savefig(curr_dir / "9h. Survey Results.png", bbox_inches = 'tight', dpi = 300)
