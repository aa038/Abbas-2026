...
Input:
    1a. Observing Log.csv               # The observing log for the chosen telescope configuration (from Part a)
    1c. Orbit Fits.pkl                  # Orbituary posteriors from Part c


Output:
    1d. Detected Planets.csv            # Properties of all planets detected atleast once
    1d. Planet Posterior Samples.csv    # Final P/e posterior samples used by part e
"""
...
# Read the orbit fits
fits_path = curr_dir / "1c. Orbit Fits.pkl"
...
posterior_samples.to_csv(curr_dir / "1d. Planet Posterior Samples.csv", index=False)

print(f"Saved {len(detected)} detected planets to 1d. Detected Planets.csv")
print(f"Saved {len(posterior_samples)} posterior samples for {posterior_samples['PlanetID'].nunique()} planets to 1d. Planet Posterior Samples.csv")