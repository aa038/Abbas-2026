...
Input:
    2a. Observing Log.csv               # The observing log for the chosen telescope configuration (from Part a)
    2c. Orbit Fits.pkl                  # Orbituary posteriors from Part c


Output:
    2d. Detected Planets.csv            # Properties of all planets detected atleast once
    2d. Planet Posterior Samples.csv    # Final P/e posterior samples used by part e
"""
...
obs_df = pd.read_csv(curr_dir / '2a. Observing Log.csv') 
# ------------------------------------------------------------------------------ #
...
# Only retained exo-Earth candidates detected in at least one epoch
detected = grouped[(grouped['NDet'] >= 1) & (grouped['Rp_REarth'] >= 0.8) & (grouped['Rp_REarth'] <= 1.4)].copy()
...
# Filter out the exo-Earths
detected = detected[detected['HZ']].copy()
# ---------------------------------------------------------- #

detected.to_csv(curr_dir / "2d. Detected Planets.csv", index = False)
...
# Read the orbit fits
fits_path = curr_dir / "2c. Orbit Fits.pkl"
...
posterior_samples.to_csv(curr_dir / "2d. Planet Posterior Samples.csv", index=False)

print(f"Saved {len(detected)} detected planets to 2d. Detected Planets.csv")
print(f"Saved {len(posterior_samples)} posterior samples for {posterior_samples['PlanetID'].nunique()} planets to 2d. Planet Posterior Samples.csv")