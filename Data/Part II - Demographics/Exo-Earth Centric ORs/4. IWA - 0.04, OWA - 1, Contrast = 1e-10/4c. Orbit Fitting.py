"""
Orbit fitting the exo-Earth candidates retained after epoch 3
---------------------------------------------------
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>> USER-TUNABLE LIMITS <<<<<<<<<<<<<<<<<<<<<<<<<< #
obs_log_file     = "4a. Observing Log.csv"   # Observing Log from Part a.
output_file_name = "4c. Orbit Fits.pkl"
cadence_name     = "Adaptive, C: 1e-10"
...
def fit_all_planets_in_log(log_path, cadence_name):
    ...
    if obs_df.empty:
        raise RuntimeError("No retained exo-Earth candidates were found in the observing log.")
