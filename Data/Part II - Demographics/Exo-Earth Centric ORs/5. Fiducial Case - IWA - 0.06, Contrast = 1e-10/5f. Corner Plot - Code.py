...
Input:
    5e. Fit, N = 1e4.csv        # MCMC posteriors from Part 5
    5f2. Beta Reference.json    # Beta approximation to the injected Rayleigh law


Output:
    5f. Corner Plot.png         # Corner plot for the posteriors
"""
...
df = pd.read_csv(curr_dir / '5e. Fit, N = 1e4.csv')
# ------------------------------------------------------------------------------ #
...
total_radius_range = [0.8, 1.4]
total_period_range = [0.03, 6.0]
...
parameters = df[['alpha', 'beta', 'gamma', 'e_alpha', 'e_beta', 'eta_Earth']]

# Define parameter labels with LaTeX formatting
param_labels = ["$\\alpha$", "$\\beta$", "$\\gamma$", "$e_{\\alpha}$", "$e_{\\beta}$", "$\\eta_{Earth}$"]

# Define axis labels
x_labels = ["Radius Index", "Period Index", "Stellar Mass Index", "$e_{\\alpha}$", "$e_{\\beta}$", "Exo-Earth OR (%)"]
y_labels = ["Radius Index", "Period Index", "Stellar Mass Index", "$e_{\\alpha}$", "$e_{\\beta}$", "Exo-Earth OR (%)"]
...
true_vals = [alpha, beta, gamma, e_alpha, e_beta, eta_Earth]
...
plt.savefig(curr_dir / '5f. Corner Plot.png', dpi=300, bbox_inches='tight')
