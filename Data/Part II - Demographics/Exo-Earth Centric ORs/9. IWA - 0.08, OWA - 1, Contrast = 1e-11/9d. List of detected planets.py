...
Input:
    9a. Observing Log.csv               # The observing log for the chosen telescope configuration (from Part a)
    9c. Orbit Fits.pkl                  # Orbituary posteriors from Part c


Output:
    9d. Detected Planets.csv            # Properties of all planets detected atleast once
    9d. Planet Posterior Samples.csv    # Final P/e posterior samples used by part e
"""
...
obs_df = pd.read_csv(curr_dir / '9a. Observing Log.csv') 
# ------------------------------------------------------------------------------ #
...
detected.to_csv(curr_dir / "9d. Detected Planets.csv", index = False)
...
# Read the orbit fits
fits_path = curr_dir / "9c. Orbit Fits.pkl"
...
posterior_samples.to_csv(curr_dir / "9d. Planet Posterior Samples.csv", index=False)

print(f"Saved {len(detected)} detected planets to 9d. Detected Planets.csv")
print(f"Saved {len(posterior_samples)} posterior samples for {posterior_samples['PlanetID'].nunique()} planets to 9d. Planet Posterior Samples.csv")