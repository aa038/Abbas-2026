...
MCMC_STEPS     = 10000          # No of orbit steps taken by the MCMC sampler (Time complexity scales as O(N)) 
...
def run_mission(save_csv=True, verbose=True):
    ...
    if save_csv:
        ...
        out.to_csv(curr_dir / "5a. Observing Log.csv", index=False)
        if verbose:
            print(f"\nSaved {len(out)} rows to 5a. Observing Log.csv")
