...
Observing Log
---------------------------------------------------
Runs a uniform revisit cadence on the on the SAG13 input population.
...
Inputs:
    2. Data / Planet Generation / SAG13 Planet Catalog.csv   

Outputs:
    5a. Observing Log.csv           # Visit-by-visit log with detections
"""
...
CONTRAST_FLOOR  = 1e-10        # Deeper of the two contrast floors used in the ExEP Mission Star List (Mamajek & Stapelfeldt 2024)
...
data_dir = curr_dir.parent / "Planet Generation"
...
while mission_clock < mission_end:
    ...
    eligible_stars = stars_df[(stars_df['NObs'] < maxObs) & (mission_clock >= stars_df['LastObs'] + timeStep)]
...
ObsLog.to_csv(curr_dir / "5a. Observing Log.csv", index=False)