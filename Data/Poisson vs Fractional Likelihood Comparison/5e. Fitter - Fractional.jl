...
Input:
    5b. 4D Tongue Plot.npz                    # The 4D tongue plot stored a 4D NumPy array (From Part b)
    5d. Planet Posterior Samples - XXX.csv    # P/e posterior samples (From Part d); XX - zero/half-bin/one-bin/two-bin
...
const parent_dir   = dirname(curr_dir)
...
"""
    ...
    # This will be a 4D array of shape 41 x 101 x 41 x 98
...
"""
    ...
    for planet_samples in groupby(posterior_samples, :PlanetID)
        ...
        for ((per_bin, ecc_bin), count) in sort(collect(bin_counts))
            ...
            push!(weights, posterior_bin_probability)
        end
...
posterior_samples_file = joinpath(curr_dir, "5d. Artificial Planet Posteriors - two-bins.csv")
...
posterior_bin_indices, posterior_bin_weights, posterior_planet_ids = create_posterior_event_weights(posterior_samples, di_bin_edges)

println("Loaded posterior likelihood contributions for $(length(posterior_planet_ids)) planets")


# ----------------------------  MCMC Fitting  ---------------------------------- #
@everywhere function compute_loglike_posterior(lam, event_bin_indices, event_bin_weights)
"""
Marginalized inhomogeneous-Poisson likelihood for uncertain planet properties.

The first term is minus the expected number of detected planets. Each observed
planet then contributes the log of its population intensity averaged over its
importance-corrected Orbituary posterior. No fractional Poisson counts are
created: every planet remains one catalog event.
"""

    lam_flat = vec(lam)
    loglike = -sum(lam_flat)

    @inbounds for planet_idx in eachindex(event_bin_indices)
        indices = event_bin_indices[planet_idx]
        weights = event_bin_weights[planet_idx]
        event_rate = zero(eltype(lam_flat))

        for sample_bin_idx in eachindex(indices)
            event_rate += weights[sample_bin_idx] * lam_flat[indices[sample_bin_idx]]
        end
...
@everywhere @model function power_law_model(event_bin_indices, event_bin_weights, di_tplot, di_stellar_masses, di_analysis_regions, di_bin_edges)
"""
MCMC Driver 
"""
    ...
    di_loglike = compute_loglike_posterior(
        di_predictions_detected,
        event_bin_indices,
        event_bin_weights
    )
...
CSV.write(joinpath(curr_dir, "5e. Fit, N = 1e4 - two-bins.csv"), df)
