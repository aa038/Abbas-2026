...
Input:
    ...
    5d. Detected Planets.csv    # The list of detected planets with their physical properties (From Part 4)
...
"""
Function to read the detected planets (Result from part 4) 
"""
...
"""
Function to place the detected planets in a 4D array that has the same dimensions as the binner tongue plot
"""
...
"""
Function to create a 4D array with the same shape as the tongue plot,
and call place_directly_imaged_planets!() to store the detected planets for fitting
"""
    # Find the length of the per and rad grids to create a 4D array to place observed planets in.
    ...
    # 4D array with the same size as the tongue plot
...
"""
Function to compute the log-likelihood for the OR model (lam) compared to the planet detectionss (obs)
"""
    ...
    @inbounds for i in 1:length(obs)
        # If there are observations of planets in a given bin, compute the log-likelihood
        # Poisson log-pdf => k*log(lam) - lam - logfactorial(k)
        if lam[i] > 0
            k = Int(obs[i])    # k = Number of actual observations in the bin
            loglike += k * log(lam[i]) - lam[i] - logfactorial(k)
        else
            # If lam=0 (no planets predicted) but obs>0 (non-zero planet detections) => impossible
            if obs[i] > 0
                return -Inf
            end
            # If k = 0, there's no data to constrain the model in that bin
            # Chuck 1000 planets in there, or 2. Knock yourself mate!
        end
    end
    return loglike
end
...
"""
    ...
    # Compute log-likelihood for this model, compared to the actual detections
