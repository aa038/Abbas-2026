...
in_hz = ((planets_df['sma_AU'] * (1 - planets_df['ecc']) >= hz_inner) &
         (planets_df['sma_AU'] * (1 + planets_df['ecc']) <= hz_outer))

# Radius between 0.8-1.4 R_E
exoEarths = in_hz & ((planets_df['Rp_Rearth'] > 0.8) & (planets_df['Rp_Rearth'] < 1.4))
# --------------------------------------------------------------------- #


# -----------------------------  Plotting  ---------------------------- #
# Background planets
plt.scatter(planets_df.loc[~exoEarths, 'P_yr'], planets_df.loc[~exoEarths, 'Rp_Rearth'], s=12, facecolors='C0', edgecolors='none', alpha=0.7, zorder=1)

# Exo-Earths highlighted in muted green
plt.scatter(planets_df.loc[exoEarths, 'P_yr'], planets_df.loc[exoEarths, 'Rp_Rearth'], s=45, facecolors='#7fbc41', edgecolors='black', linewidths=1.2, zorder=3)
...
plt.savefig(curr_dir / "fig1_Period_vs_Radius.png", dpi=300, bbox_inches='tight')
# ---------------------------------------------------------------------- #