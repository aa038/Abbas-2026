# Figure 11 -- Comparing Input vs Fitted Eccentricities
This folder reproduces **Figure 11** from Abbas et al. (2026, ApJ), and compares the input Rayleigh distribution for eccentricities with the OR model, which was fit with a Beta function.

## Files
- `Compare Ecc Dist.py`  -- Script to generate Fig 11
- `fig11_EccentricityDist.png` -- Output figure (matches Fig 11 in the paper).
...
- The results of all these scripts are provided as data products in the directory ``Data/Part II - Demographics/5. Fiducial Case - IWA - 0.06, Contrast = 1e-10`. `Compare Ecc Dist.py` references these and can thus be run directly to reproduce Fig 11.
...
```bash
python "Fig 11 - Eccentricity Dist/Compare Ecc Dist.py"
```
This will regenerate `fig11_EccentricityDist.png`