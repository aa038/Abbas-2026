...
Output:
    fig12_3x3_tplot.png      # Matches Fig 12
"""
...
for idx, run_dir_name in enumerate(run_dirs):
    ...
    im = ax.imshow(completeness_2d,
                   ...
                   cmap=ylrdblu_cmap, vmin = 0, vmax = 98)
    
    # Smooth and draw contours
    smoothed      = gaussian_filter(completeness_2d, sigma=3.0)
    star_contours = levels = [1, 40, 80, 92]      # Number of stars for which contour lines are drawn
    colors = ['#8A2BE2', '#48D1CC', '#00FFFF', 'deeppink']
    labels = ['1 star', '40 stars', '80 stars', '92 stars']
...
plt.savefig(curr_dir / 'fig12_3x3_tplot.png', dpi=300, bbox_inches='tight')
plt.show()