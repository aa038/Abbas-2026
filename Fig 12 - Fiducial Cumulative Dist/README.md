# Figure 12 -- Fiducial CDF
This folder reproduces **Figure 12** from Abbas et al. (2026, ApJ), and plots the the mock survey CDFs. The mock surveys were generated using random draws from the MCMC posteriors

## Files
- `CDF Plot.py`  -- Script to generate Fig 12
- `fig12_CumulativeDist.png` -- Output figure (matches Fig 12 in the paper).
...
- The result of `5g. Mock Survey - Code.py` is **NOT PROVIDED** since it exceeds the 200MB file size limit. To remake `fig12_CumulativeDist.png`, `5g. Mock Survey - Code.py` will have to be **RUN FIRST**. It can be run as is without any changes.
...
```bash
python "Fig 12 - Fiducial Cumulative Dist/CDF Plot.py"
```
This will regenerate `fig12_CumulativeDist.png`