...
Output:
    fig16b_EOR_CumulativeDist.png        # Plot of the CDFs
"""
import numpy as np
import pandas as pd
...
bins = np.arange(0, max(num_planets) + 2, 25)
...
bins = np.arange(0, max(num_HZ_planets) + 2, 4)
...
axs[5].set_xlim([75,175])

axs[5].xaxis.set_major_locator(ticker.MultipleLocator(20))
axs[5].xaxis.set_minor_locator(ticker.MultipleLocator(10))

axs[5].yaxis.set_major_locator(ticker.MultipleLocator(100))
axs[5].yaxis.set_minor_locator(ticker.MultipleLocator(50))
# ---------------------------------------------------------------------------- #

plt.savefig(curr_dir / "fig16b_EOR_CumulativeDist.png", bbox_inches = 'tight', dpi = 300)
