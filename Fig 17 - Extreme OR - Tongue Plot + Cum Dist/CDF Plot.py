...
-------------------------------------------
Script to plot the mock survey CDFs for the scaled OR realisation

Input:
    Data/Part II - Demographics/Scaled Occurrence Rates/5. IWA - 0.06, OWA - 1, Contrast = 1e-10/5d. Detected Planets.csv      # List of detected planets (From Part 4)
    Data/Part II - Demographics/Scaled Occurrence Rates/5. IWA - 0.06, OWA - 1, Contrast = 1e-10/5g. Mock Survey.csv           # The full list of mock surveys (From Part 7)


Output:
    fig17b_EOR_CumulativeDist.png        # Plot of the CDFs
"""
...
data_dir = curr_dir.parent / "Data" / "Part II - Demographics" / "Scaled Occurrence Rates" / "5. IWA - 0.06, OWA - 1, Contrast = 1e-10"
...
for sim_id in range(0, 2000):
    ...
    if sim_id in grouped.groups:
        ...
        num_HZ_planets.append(len(group[(group['HZ'] == True) & (group['planet_rad'] > 0.8) & (group['planet_rad'] < 1.4)]))
    else:
        # No planets detected in this simulation
        planet_rad_cumulative.append([0])
        per_cumulative.append([0])
...
bins = np.arange(0, max(num_planets) + 2, 30)
...
bins = np.arange(0, max(num_HZ_planets) + 2, 10)
...
axs[5].set_xlim([100, 300])

axs[5].xaxis.set_major_locator(ticker.MultipleLocator(50))
axs[5].xaxis.set_minor_locator(ticker.MultipleLocator(25))

axs[5].yaxis.set_major_locator(ticker.MultipleLocator(100))
axs[5].yaxis.set_minor_locator(ticker.MultipleLocator(50))
# ---------------------------------------------------------------------------- #

plt.savefig(curr_dir / "fig17b_EOR_CumulativeDist.png", bbox_inches = 'tight', dpi = 300)
