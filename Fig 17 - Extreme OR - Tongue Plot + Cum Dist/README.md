# Figure 17 -- Scaled OR
This folder reproduces **Figure 17** from Abbas et al. (2026, ApJ), and plots the planets realised from an exaggerated SAG13 OR overlaid on the fiducial tongue plot, and the CDFs for the associated mock surveys.

## Files
- `Plot Planets on Tongue Plot.py`  -- Script to generate the tongue plot with the overlaid planets
- `fig17a_EOR_tplot.png`            -- Output figure (matches Fig 17a in the paper).
- `CDF Plot.py`                     -- Script to plot the CDFs for the mock surveys
- `fig17b_EOR_CumulativeDist.png`   -- Output figure (matches Fig 17b in the paper).
...
- The result of `5g. Mock Survey - Code.py` is **NOT PROVIDED** since it exceeds 200MB. To remake `fig17b_EOR_CumulativeDist.png`, `5g. Mock Survey - Code.py` will have to be **RUN FIRST**. It can be run as is without any changes.
...
```bash
python "Fig 17 - Extreme OR - Tongue Plot + Cum Dist/Plot Planets on Tongue Plot.py"
python "Fig 17 - Extreme OR - Tongue Plot + Cum Dist/CDF Plot.py"
```
This will regenerate the figures `fig17a_EOR_tplot.png` and `fig17b_EOR_CumulativeDist.png` respectively.