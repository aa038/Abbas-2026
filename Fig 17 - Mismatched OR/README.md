# Figure 17 -- Mismatched CDF
This folder reproduces **Figure 17** from Abbas et al. (2026, ApJ), and plots the the mock survey CDFs for an OR planet mass and sma instead of radius and period. The mock surveys were generated using random draws from the MCMC posteriors

## Files
- `CDF Plot.py`  -- Script to generate Fig 17
- `fig17_MismatchedModel.png` -- Output figure (matches Fig 17 in the paper).
...
- The result of `5g. Mock Survey - Code.py` is **NOT PROVIDED** since it exceeds the 200MB file size limit. To remake `fig17_MismatchedModel.png`, `5g. Mock Survey - Code.py` will have to be **RUN FIRST**. It can be run as is without any changes.
...
```bash
python "Fig 17 - Mismatched OR/CDF Plot.py"
```
This will regenerate `fig17_MismatchedModel.png`