...
MIN_FRAC_GAP   = 0.25           # Minimum time to wait before scheduling an adaptie epoch (scaled by the HZ period, see Eqn 5) (in years)
MAX_FRAC_GAP   = 1.00           # Maximum time to wait before scheduling an adaptie epoch (scaled by the HZ period, see Eqn 5) (in years)
...
def detectability_fraction(sep_arcsec, r3D_AU, phase_func, Rp_RE, albedo, IWA = IWA, OWA = OWA, contrast_floor = CONTRAST_FLOOR):
    ...
    det_mask_sep = sep_mask    # Consider using this if planet radius and albedo is not known
    ...
    det_frac_sep = det_mask_sep.sum(axis=0) / N

    return det_frac, det_frac_sep
...
def objective_unclassified(orbit_fits, L_sol, d_pc, t_grid, Rp_Re, albedo):
    ...
    p_det, p_geom = detectability_fraction(sep, r3D, phase, Rp_Re, albedo)
    ...
    if has_hz and has_nohz:
        ...
        return J, weighted_sep, p_det, p_geom
...
def schedule_next_observation_for_star(star_name, mission_now, aggregate_mode=False):
    ...
    for _, p in planets.iterrows():
        ...
        if classified:
            J, Delta, p_det, p_geom = objective_classified(orbit_fits, L_sol, d_pc, t_grid, Rp_Re, alb)
        else:
            J, Delta, p_det, p_geom = objective_unclassified(orbit_fits, L_sol, d_pc, t_grid, Rp_Re, alb)
    ...
    if not aggregate_mode:
        ...
        # Retrieve the metric arrays for the planet that actually set the next epoch
        best_res = next(res for res in per_planet_results if res["PlanetID"] == pid_best)

        # Number of visits already completed before choosing the next one
        # Example: after epoch 1, this is 1
