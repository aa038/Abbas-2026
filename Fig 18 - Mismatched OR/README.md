# Figure 18 -- Mismatched CDF
This folder reproduces **Figure 18** from Abbas et al. (2026, ApJ), and plots the the mock survey CDFs for an OR planet mass and sma instead of radius and period. The mock surveys were generated using random draws from the MCMC posteriors

## Files
- `CDF Plot.py`  -- Script to generate Fig 18
- `fig18_MismatchedModel.png` -- Output figure (matches Fig 18 in the paper).
...
- The result of `5g. Mock Survey - Code.py` is **NOT PROVIDED** since it exceeds the 200MB file size limit. To remake `fig18_MismatchedModel.png`, `5g. Mock Survey - Code.py` will have to be **RUN FIRST**. It can be run as is without any changes.
...
```bash
python "Fig 18 - Mismatched OR/CDF Plot.py"
```
This will regenerate `fig18_MismatchedModel.png`