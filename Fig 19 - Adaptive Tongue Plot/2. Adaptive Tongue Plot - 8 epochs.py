...
Input:
    1. AC Observing Log - 1e-10.csv       # The observing log for te fiducial configuration generated with an adaptive cadence


Output:
    2. 4D Tongue Plot.npz                 # The 4D tongue plot stored a 4D NumPy array
"""
...
def build_epoch_lookup_from_obslog(obslog, stars_df, max_epochs=N_EPOCHS):
    ...
    for star_name in stars_df["HDName"]:
        times = (
            obslog.loc[obslog[star_col] == star_name, time_col]
            .dropna()
            .astype(float)
            .to_numpy()
        )
...
if __name__ == '__main__':
    ...
    obslog = pd.read_csv(curr_dir / '1. AC Observing Log - 1e-10.csv')
