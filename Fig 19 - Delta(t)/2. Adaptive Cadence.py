"""
Illustrative System - Adaptive Cadence (Part 4 of 5)
---------------------------------------------------
Runs an adaptive cadence on the demo short-period, inclined planet
(Fig. 3b). Loops through mission time, selects the next eligible target, calls
...
# - The detection probability (p_det in Eqn 5, Abbas et al 2026) is computed in the function detectability_fraction() 
...
# It is COMPUTATIONALLY DEMANDING and is typically only run when reproducing Fig. 3b,
...
def objective_unclassified(orbit_fits, L_sol, d_pc, t_grid, Rp_Re, albedo):
    ...
    For an unclassified planet (Unclassified = 100% of orbital posteriors classified as fully HZ/non-HZ):
        ...
        - If only one subset exists: internal spread x detectability (fallback).nd
...
# ------------------------- Section 5: Scheduler ---------------------------- #
def build_time_grid(last_obs, mission_end, period_char, min_frac = MIN_FRAC_GAP, max_wait_years=MAX_WAIT_YEARS, slot_days = TIME_SPACING):
    """
...
def schedule_next_observation_for_star(star_name, mission_now, aggregate_mode=False):
    ...
    for _, p in planets.iterrows():
        ...
        if classified:
            J, p_det, Delta = objective_classified(orbit_fits, L_sol, d_pc, t_grid, Rp_Re, alb)
        else:
            J, p_det, Delta = objective_unclassified(orbit_fits, L_sol, d_pc, t_grid, Rp_Re, alb)
    ...
    if not aggregate_mode:
        ...
        best_res = next(result for result in per_planet_results if result["PlanetID"] == pid_best)

        sched_epoch = int(s_row["NObs"])

        np.savez(
            curr_dir / f"2. Diagnostics - Epoch {sched_epoch:02d}.npz",
            epoch=sched_epoch,
            planet_id=pid_best,
            t_grid=t_grid,
            Delta=best_res["Delta"],
            p_geom=best_res["p_det"],
            J=best_res["J"],
            t_selected=t_best,
            idx_selected=idx_best,
            last_obs=last_obs,
            classified=best_res["classified"],
            hz_confidence=best_res["hz_confidence"],
            guardrail_met=ok_best,
            p_geom_min=P_DET_MIN,
        )
...
planets_df = pd.read_csv(curr_dir / "1. Planet Catalog w Mass.csv")
# --------------------------------------------------------------------------- #
...
def run_mission(save_csv=True, verbose=True):
    ...
    if save_csv:
        ...
        out.to_csv(curr_dir / "4. Observing Log.csv", index=False)
        if verbose:
            print(f"\nSaved {len(out)} rows to 4. Observing Log.csv")
