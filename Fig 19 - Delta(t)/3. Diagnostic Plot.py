...
outpath  = curr_dir / "fig19_AC_Diagnostics.png"
# --------------------------------------------------------------------------- #
...
for i, file in enumerate(files):
    ...
    p_geom     = data["p_geom"]
    J          = data["J"]
    classified = bool(data["classified"])
    last_obs   = float(data["last_obs"])
    idx_best   = int(data["idx_selected"])

    Delta  = normalized(Delta)
    p_geom = normalized(p_geom)
    J      = normalized(J)
