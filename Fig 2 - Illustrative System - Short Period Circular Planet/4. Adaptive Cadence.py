...
---------------------------------------------------
Runs an adaptive cadence on the demo short-period, near face-on planet
(Fig. 2b). Loops through mission time, selects the next eligible target, calls
...
# It is COMPUTATIONALLY DEMANDING and is typically only run when reproducing Fig. 2b,
...
def observe_star_once(star_name: str, t_year: float):
    ...
    for _, p in planets.iterrows():
        ...
        if det:
            ...
            Sep_arcsec = rng.normal(Sep_true, SEP_UNCERTAINTY)
            PA_deg     = rng.normal(PA_true, pa_uncertainty_deg) % 360.0
