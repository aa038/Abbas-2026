"""
Corner Plot for the OR parameters (Part 6 of 9)
-------------------------------------------
This script generates a corner plot for the results of the MCMC OR fit.

Input:
    Data / Poisson vs Fractional Likelihood Comparison / 5e. Fit, N = 1e4 - zero.csv        # The full list of MCMC posteriors for zero-width posteriors
    Data / Poisson vs Fractional Likelihood Comparison / 5e. Fit, N = 1e4.csv               # The full list of MCMC posteriors with Poisson likelihood

Output:
    fig20_CornerPlotComparison.png                # Corner plot for the posteriors
"""
...
def hz_fraction(row, stellar_mass=1.0, stellar_luminosity=1.0):
    ...
    if abs(exponent) < 1e-6:
        ...
        period_fraction[valid] = np.log(period_high[valid] / period_low[valid]) / denominator
    
    else:
        denominator = PERIOD_MAX**exponent - PERIOD_MIN**exponent
        
        period_fraction[valid] = (period_high[valid]**exponent - period_low[valid]**exponent) / denominator
...
# Load the MCMC posteriors for the Poisson likelihood and P/e posteriors with zeo-width
df_poisson = pd.read_csv(data_dir / '5e. Fit, N = 1e4.csv')
df_zero    = pd.read_csv(data_dir / '5e. Fit, N = 1e4 - zero.csv')
# ------------------------------------------------------------------------------ #

# ---------------------  Exo-Earth OR for a 1M_sol star  ----------------------- #
alpha_plus_one = df_poisson['alpha'] + 1
beta_plus_one  = df_poisson['beta'] + 1

# Restrict the full-population normalization to 0.8-1.4 R_E
df_poisson['freq_earth_radius'] = df_poisson['freq'] * (1.4 ** (df_poisson['alpha']+1) - 0.8 ** (df_poisson['alpha']+1)) / (3.4 ** (df_poisson['alpha']+1) - 0.5 ** (df_poisson['alpha']+1))

# Compute the exo-Earth OR for a 1 M_sol star
df_poisson['eta_Earth'] = [100.0 * row['freq_earth_radius'] * hz_fraction(row) for _, row in df_poisson.iterrows()]

alpha_plus_one = df_zero['alpha'] + 1
beta_plus_one  = df_zero['beta'] + 1

# Restrict the full-population normalization to 0.8-1.4 R_E
df_zero['freq_earth_radius'] = df_zero['freq'] * (1.4 ** (df_zero['alpha']+1) - 0.8 ** (df_zero['alpha']+1)) / (3.4 ** (df_zero['alpha']+1) - 0.5 ** (df_zero['alpha']+1))

# Compute the exo-Earth OR for a 1 M_sol star
df_zero['eta_Earth'] = [100.0 * row['freq_earth_radius'] * hz_fraction(row) for _, row in df_zero.iterrows()]
# ------------------------------------------------------------------------------ #
...
eta_Earth = 100.0 * eta_Earth * hz_fraction(reference)
    ...
    for i in range(n_params):
        for j in range(n_params):
            ...
            # Diagonal: Plot 1D histograms
                ...
                ax.hist(param_data, bins=30, color='black', histtype='step', density=True, edgecolor='black', linestyle=ls)
            ...
            # Lower triangle: Plot 2D contours
                ...
                ax.contour(x_grid, y_grid, z, levels=levels, colors=contour_colors, linewidths=1, linestyles=ls)
...
corner_plot(df_poisson, ls = "-")
corner_plot(df_zero, ls = "--")
...
legend_elements = [
    # Posterior identity
    Line2D([0], [0], color='black', lw=2, ls='-', label='Poisson likelihood'),
    Line2D([0], [0], color='black', lw=2, ls='--', label='Marginalized likelihood'),

    # Credible levels
    Line2D([0], [0], color='red', lw=1, label=r'$68\%$'),
    Line2D([0], [0], color='blue', lw=1, label=r'$95\%$'),
    Line2D([0], [0], color='green', lw=1, label=r'$99.7\%$'),
]

fig.legend(handles=legend_elements, bbox_to_anchor=(0.85, 0.85), fontsize=12)
# ------------------------------------------------------------------------------ #

plt.savefig(curr_dir / 'fig20_CornerPlotComparison.png', dpi=300, bbox_inches='tight')
