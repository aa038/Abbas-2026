...
Generates a single long-period, nearly edge-on planet on an eccentric orbit 
around one star (the first entry of HWO Stars.csv). This illustrative system 
is used for Figure 4 (uniform vs adaptive cadence demo).

Output:
    1. Planet Catalog.csv

Notes:
- Fixed planet parameters (Rp=1 R_E, P=2 yr, ecc=0.2, inc=88.06).
...
for i, row in stars.iterrows():
    ...
    P            = 2
    ecc          = 0.2
    inc          = 1.537  # in radians = 88.06 degrees
