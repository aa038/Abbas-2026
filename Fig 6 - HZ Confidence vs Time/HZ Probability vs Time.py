...
for i, (label, path) in enumerate(cadences.items()):
    ...
    times_68 = np.array(times_68)
    times_95 = np.array(times_95)
    ...
    axs[0].step(times_68-2035, true_positives_68, label=f"{label}", ls = ls[i])

    #axs[1].step(times_95, counts_95, label=f"{label} (TP + FP)")
    axs[1].step(times_95-2035, true_positives_95, label=f"{label}", ls = ls[i])
    # ---------------------------------------------------------- #
...
axs[0].set_xlabel("Mission Time (yr)")
...
axs[1].set_xlabel("Mission Time (yr)")
axs[1].set_ylim([0, 25])
axs[1].set_xlim([0, 10])
...
for i, label in enumerate(cadences.keys()):
    ...
    axins0.step(t68-2035, tp68, ls=ls[i])
    axins1.step(t95-2035, tp95, ls=ls[i])
