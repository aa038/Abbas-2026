...
---------------------------------------------------
Runs an adaptive cadence on the all the SAG13 planets generated in the study.
...
checks (IWA/OWA + contrast (if allowed)), and logs each visit.

Inputs:
    1. Data / Planet Generation SAG13 Planet Catalog

Outputs:
    3. 1. AC Observing Log - 1e-10.csv
...
output_file_name = "1. AC Observing Log - 1e-10.csv"
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< #
...
data_dir = curr_dir.parent / "Data" / "Planet Generation" 
...
planets_df['NDet']         = 0
stars_df['ActivePlanetID'] = pd.NA
stars_df['ActiveLocked']   = False
...
def run_mission(save_csv=True, verbose=True):
    ...
    if save_csv:
        ...
        out.to_csv(curr_dir / f"{output_file_name}", index=False)
        if verbose:
