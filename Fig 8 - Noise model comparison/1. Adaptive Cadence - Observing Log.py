"""
Observing Log - Noise Model
---------------------------------------------------
Generates an observing log of the input SAG13 population with the fiducial configuration,
with a noise model that imposes an additional exposure time constraint.

The resulting log has 3 extra columns that indicate if the planet would have been 
detected with a 3, 12 and 24 hr exposure, along the noise-free IWA/contrast floor condition.
...
Outputs:
    1. Observing Log.csv           # Visit-by-visit log with detections
"""
...
# It is COMPUTATIONALLY DEMANDING and is typically only run when experimenting with alternative cadence strategies.
...
CONTRAST_FLOOR = 1.0e-10        # Contrast floor for this fiducial-case directory
...
# Exposure times evaluated on the same adaptive observing schedule
EXPOSURE_HOURS   = (3.0, 12.0, 24.0)
VISIT_BLOCK_DAYS = 1.0
...
def separation_dependent_raw_contrast(sep_arcsec, iwa=IWA, contrast_floor=CONTRAST_FLOOR, 
    contrast_at_iwa_factor=3.0, rolloff_width_iwa=0.5, rolloff_power=2.0):
    """
...
def count_rates(V_mag, planet_star_contrast, sep_arcsec):
    ...
    Csp = SPECKLE_STABILITY * C_speckle / POSTPROCESSING_GAIN
...
def detectionCheck(DetParams, albedo, Rp_m, V_mag, t_visit_s=np.inf):
    """
...
def observe_star_once(star_name: str, t_year: float):
    ...
    d_pc, L_sol, M_sol, V_mag = s_row['d_pc'], s_row['L_sol'], s_row['M_sol'], s_row['V']
    ...
    for _, p in planets.iterrows():
        ...
        det_ideal = int(sepCheck(Sep_true) and contrastCheck(PhaseFunc, Rp_Re, r3D_AU, albedo))
        DetParams = [Sep_true, PA_true, r3D_AU, PhaseFunc]

        _, planet_ctr, t_req_s, Cp, Cb, Csp = detectionCheck(DetParams, albedo, Rp_Re * EARTHRADII, V_mag)

        # Evaluate every exposure time on this exact same visit
        det_etc_3hr  = int(t_req_s <= 3.0 * 3600.0)
        det_etc_12hr = int(t_req_s <= 12.0 * 3600.0)
        det_etc_24hr = int(t_req_s <= 24.0 * 3600.0)
        ...
        new_rows.append([p['PlanetID'], star_name, sma, ecc, Rp_Re, Mp_Me, d_pc, M_sol, L_sol, Sep_arcsec, PA_deg, r3D_AU, PhaseFunc,
            planet_ctr, t_req_s, t_req_s / 3600 if np.isfinite(t_req_s) else np.inf, Cp, Cb, Csp, det_ideal, det_etc_3hr, det_etc_12hr, 
            det_etc_24hr, det,
...
ObsLog = pd.DataFrame(columns = ['PlanetID', 'StarName', 'SMA_AU', 'ecc', 'Rp_REarth', 'Mp_MEarth', 'd_pc', 'M_sol', 'L_sol', 'Sep', 'PA', 
                                 ...
                                 'DetStatus_ETC_3hr', 'DetStatus_ETC_12hr', 'DetStatus_ETC_24hr', 'DetStatus', 'NDet', 'NObs', 'LastObs'])
...
def run_mission(save_csv=True, verbose=True):
    ...
    while (mission_clock <= MISSION_END) and (not all_done()):
        ...
        mission_clock += VISIT_BLOCK_DAYS / 365.0
    ...
    if save_csv:
        ...
        out.to_csv(curr_dir / f"1. Observing Log.csv", index=False)
        if verbose:
            print(f"\nSaved {len(out)} rows to 1. Observing Log.csv")
