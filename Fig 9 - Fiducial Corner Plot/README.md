# Figure 9 -- Fiducial Corner Plot
This folder reproduces **Figure 9** from Abbas et al. (2026, ApJ), generating a corner plot for the results of the MCMC occurrence rate fit.

## Files
- `Corner Plot.py`  -- Script to generate Fig 9
- `fig9_CornerPlot.png` -- Output figure (matches Fig 9 in the paper).
...
- Requires the results of scripts `a-e` in `Data/Part II - Demographics/5. Fiducial Case - IWA - 0.06, Contrast = 1e-10`: 
...
- The results of all these scripts are provided as data products in the directory ``Data/Part II - Demographics/5. Fiducial Case - IWA - 0.06, Contrast = 1e-10`. `Corner Plot.py` references these and can thus be run directly to reproduce Fig 9.
...
```bash
python "Fig 9 - Fiducial Corner Plot/Corner Plot.py"
```
This will regenerate `fig9_CornerPlot.png`